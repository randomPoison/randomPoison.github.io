...
Up to this point I've tried to keep my personal opinions out of my summary of the technical issues, but I'm sure my frustration can be seen seeping through the cracks. Overall, my assessment of the situation is that code sharing between Unity and .NET works just well enough to be tempting to use, while being broken enough to present major obstacles to large scale development.

The fundamental issue is that, while the two are similar on the surface, is that Unity is not a true .NET environment. On some platforms Unity uses Mono to run your code, which at least means that your code will behave like regular C# at runtime, but IL2CPP is a whole separate thing that doesn't work like any of the "real" .NET environments. Combine that with the fact that Unity has a completely bespoke solution for dealing with dependency management and has to handle certain platform-specific issues that other .NET runtimes don't (most notably ahead-of-time compilation and running in web browsers), and the picture we end up with is one where Unity is not one of many valid .NET runtimes, but it's own separate thing that happens to be largely (but not completely!) source-compatible with .NET.

Source compatibility is the notable thing here. Being able to copy a piece of code between a Unity project and a .NET project sure *feels* like sharing code, so it's awfully tempting to say that code sharing is possible. And, as noted above, it is possible (easy, even!) at a small scale. What worries me is that nothing about this setup seems scalable.
...
Trying to treat Unity and .NET as compatible code environments is a hazard; It's a recipe for tech debt that will build up slowly over time, appearing to work long enough for you dig yourself into a hole too deep to climb back out of. If you can be certain that you only need to share small, isolated pieces of game logic between your client and server, then maybe it'll work for you. But if you're looking for a robust, scalable solution for sharing core application logic and simplifying your game's architecture, I recommend you keep looking because this isn't it.